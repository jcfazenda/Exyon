...
namespace Api.Domain.Repository
{
    public class Repository<TEntity, TID> : IRepository<TEntity, TID> where TEntity : class
    {
        ...
        public Repository(BancoDadosContext context)
        {
            Db = context;
        ...
        public virtual IQueryable<TEntity> GetAutoComplete(Expression<Func<TEntity, bool>> filter, Expression<Func<TEntity, object>> orderBy, int quantity = 10) => DbSet.Where(filter).OrderBy(orderBy).Take(quantity);
